...
import { buildCategoryAssetCandidates, CATEGORY_IMAGES_BASE_URL } from "@/data/categoryAssets";
...
export const CategoryBar = ({ selectedCategory, onSelectCategory, products = [] }: CategoryBarProps) => {
  ...
  const buildCombinedCategoryFallbacks = (categoryName: string): string[] => {
    const localFallback = categoryName === "TODOS" ? allProductsImage : undefined;
    return buildCategoryAssetCandidates(categoryName, CATEGORY_IMAGES_ENV_BASE_URL, localFallback);
  };
